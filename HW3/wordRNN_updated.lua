...
cmd:option('-seqLength',          7,                          'number of timesteps to unroll for')
...
local epoch = 1
local testPerplexity = torch.Tensor(opt.epoch)
local trainPerplexity = torch.Tensor(opt.epoch)
...
repeat
  ...
  local LossTest = evaluate(data.testData)
	...
	print('\nSampled Text:\n' .. sample('Buy low, sell high is the',7, true))
...
local range = torch.range(1, opt.epoch)
gnuplot.pngfigure(paths.concat(opt.save,'PerplexityGraph.png'))
gnuplot.plot({'testPPL',testPerplexity},{'trainPPL',trainPerplexity})
